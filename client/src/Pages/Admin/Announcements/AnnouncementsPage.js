...
function AnnouncementsPage() {
  ...
  useEffect(() => {
    const fetchWebsites = async () => {
      try {
        const res = await axios.get('http://localhost:8800/api/websites/detay'); // API'den web sitelerini çekme
  ...
  useEffect(() => {
    const fetchAllRecords = async () => {
      try {
        const res = await axios.get("http://localhost:8800/api/duyuru/first");
  ...
  const handleSearch = async () => {
    try {
      let res;
      if (searchTerm !== "") {
        res = await axios.get(`http://localhost:8800/api/duyuru/search/${searchTerm}`);
      } else {
        res = await axios.get("http://localhost:8800/api/duyuru/first");
      }
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:8800/api/duyuru/delete/${selectedRecordId}`);
