...
const ManageImages = () => {
    ...
    const fetchFiles = useCallback(async () => {
        try {
            const response = await axios.get('http://localhost:8800/api/images/files');
    ...
    const handleImageUploadToServer = async () => {
        ...
        try {
            setUploading(true);
            await axios.post('http://localhost:8800/api/images/upload', formData, {
                headers: {
    ...
    const handleDeleteFile = async (filename) => {
        try {
            await axios.delete(`http://localhost:8800/api/images/files/${filename}`);
